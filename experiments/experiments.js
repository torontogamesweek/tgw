...
//   // Complete poster positioning data (percentage-based) - 39 balls, 29 people
//   posterBalls: [
//     {x: 62.90, y: 66.69, size: 5.59, img: 1},
...
//     {x: 2.15, y: 51.70, size: 3.22, img: 6},
//     {x: 3.80, y: 48.82, size: 3.22, img: 7},
//     {x: 36.25, y: 42.08, size: 29.94, img: 8},
//     {x: 5.99, y: 51.75, size: 3.30, img: 9},
...
//     {x: 36.39, y: 42.21, size: 29.80, img: 14},
...
//     {x: 7.38, y: 24.00, size: 5.58, img: 19},
//     {x: 0.86, y: 16.71, size: 5.80, img: 20},
//     {x: 54.36, y: 87.75, size: 5.38, img: 21},
//     {x: 4.23, y: 2.43, size: 12.18, img: 22},
//     {x: 79.80, y: 2.19, size: 3.08, img: 23},
...
//     {x: 70.99, y: 64.40, size: 28.58, img: 30},
//     {x: 73.57, y: 42.95, size: 10.82, img: 31},
//     {x: 86.53, y: 34.67, size: 10.60, img: 32},
//     {x: 23.78, y: 80.48, size: 13.47, img: 33},
//     {x: 51.36, y: 66.73, size: 14.47, img: 34},
//     {x: 59.81, y: 83.83, size: 5.59, img: 35},
//     {x: 9.46, y: 57.54, size: 3.22, img: 1},
//     {x: 92.40, y: 3.56, size: 5.52, img: 2},
//     {x: 70.49, y: 11.84, size: 22.21, img: 3},
//     {x: 7.88, y: 54.72, size: 3.18, img: 4}
//   ],
...
//     {x: 69.56, y: 63.40, size: 4.30, img: 3},
//     {x: 34.24, y: 91.84, size: 4.58, img: 4},
//     {x: 56.59, y: 39.25, size: 5.66, img: 5},
//     {x: 58.81, y: 18.08, size: 5.66, img: 6},
//     {x: 40.27, y: 4.30, size: 7.74, img: 7},
//     {x: 21.92, y: 61.38, size: 14.90, img: 8},
...
//     {x: 34.81, y: 41.83, size: 4.80, img: 13},
...
//     {x: 45.77, y: 86.59, size: 9.89, img: 0},
//     {x: 36.46, y: 24.13, size: 9.53, img: 1},
//     {x: 32.88, y: 7.80, size: 7.38, img: 2},
//     {x: 90.04, y: 88.78, size: 6.30, img: 3},
//     {x: 75.50, y: 34.85, size: 5.52, img: 4},
//     {x: 74.02, y: 89.27, size: 5.73, img: 5},
//     {x: 26.29, y: 37.66, size: 5.66, img: 6},
//     {x: 39.04, y: 83.36, size: 8.81, img: 7},
//     {x: 47.13, y: 1.75, size: 6.45, img: 8},
//     {x: 85.39, y: 43.64, size: 12.32, img: 9}
//   ]
// };

// // NEW: Poster transition system
...
//       console.log('Poster elements not found');
//       return;
//     }
...
//       setTimeout(() => {
//         // Get the ACTUAL displayed dimensions of the poster image
...
//         console.log('Poster dimensions:', this.posterWidth, 'x', this.posterHeight);
...
//   getPosterPosition(xPercent, yPercent) {
//     if (!this.posterWidth) {
//       console.warn('Poster dimensions not available');
//       return {
...
//     // Exact same calculation as position mapper
//     return {
//       x: (xPercent / 100) * this.posterWidth,
//       y: (yPercent / 100) * this.posterHeight
//     };
//   },
  
//   getPosterSize(sizePercent) {
//     if (!this.posterWidth) {
//       return 50;
//     }
//     // Size is percentage of poster WIDTH
//     return (sizePercent / 100) * this.posterWidth;
//   }
// };
...
// // MODIFIED: ConnectionsGame uses poster positions
...
//     // Use poster positions if available, otherwise fallback to random
//     if (PosterTransition.posterWidth && Config.posterBalls && Config.posterPeople) {
//       console.log('Using poster positions for ConnectionsGame');
//       console.log('Poster dimensions:', PosterTransition.posterWidth, 'x', PosterTransition.posterHeight);
      
//       // Create balls from poster positions
//       Config.posterBalls.forEach((posterData, i) => {
//         const pos = PosterTransition.getPosterPosition(posterData.x, posterData.y);
//         const posterSize = PosterTransition.getPosterSize(posterData.size); // This is width/height, NOT radius
        
//         // Normalize size for gameplay: map from poster size to reasonable game size
//         // Poster sizes range from ~3% to ~30% of poster width
//         // Normalize to 6% - 14% range for better gameplay
//         const sizePercent = posterData.size;
//         const normalizedPercent = 6 + ((sizePercent - 3) / (30 - 3)) * (14 - 6);
//         const normalizedSize = PosterTransition.getPosterSize(normalizedPercent);
        
//         if (i === 0) {
//           console.log(`Ball 0: poster(${posterData.x}%, ${posterData.y}%)`);
//           console.log(`  Screen pos: (${pos.x.toFixed(1)}, ${pos.y.toFixed(1)})`);
//           console.log(`  Poster size: ${posterData.size}% = ${posterSize.toFixed(1)}px`);
//           console.log(`  Normalized size: ${normalizedPercent.toFixed(1)}% = ${normalizedSize.toFixed(1)}px`);
//         }
...
//             vx: 0,
//             vy: 0,
//             radius: posterSize / 2, // Start at exact poster size (radius = half the size)
//             targetRadius: normalizedSize / 2, // Grow/shrink to normalized size
//             ballImage: posterData.img,
//             type: "ball",
//             rotationSpeed: (Math.random() - 0.5) * 0.02,
//             rotation: 0,
//             scaleProgress: 0, // Animation progress 0-1
//             scale: 1,
//             targetScale: 1,
//             currentScale: 1,
...
//       // Create people from poster positions
//       Config.posterPeople.forEach((posterData, i) => {
//         const pos = PosterTransition.getPosterPosition(posterData.x, posterData.y);
//         const posterSize = PosterTransition.getPosterSize(posterData.size);
        
//         // Normalize people sizes similarly
//         const sizePercent = posterData.size;
//         const normalizedPercent = 6 + ((sizePercent - 4) / (15 - 4)) * (12 - 6);
//         const normalizedSize = PosterTransition.getPosterSize(normalizedPercent);
        
//         if (i === 0) {
//           console.log(`Person 0: poster(${posterData.x}%, ${posterData.y}%)`);
//           console.log(`  Screen pos: (${pos.x.toFixed(1)}, ${pos.y.toFixed(1)})`);
//           console.log(`  Poster size: ${posterData.size}% = ${posterSize.toFixed(1)}px`);
//           console.log(`  Normalized size: ${normalizedPercent.toFixed(1)}% = ${normalizedSize.toFixed(1)}px`);
//         }
...
//           personHeight: posterSize, // Start at exact poster size
//           targetHeight: normalizedSize, // Grow/shrink to normalized size
//           type: "person",
//           rotation: Math.random() * Math.PI * 2,
...
//       console.log(`Created ${this.balls.length} entities from poster (${Config.posterBalls.length} balls + ${Config.posterPeople.length} people)`);
//     } else {
//       console.log('Using random positions (poster not ready)');
//       // Fallback: Random positions (original behavior)
//       for (let i = 0; i < 8; i++) {
//         const a = Math.random() * Math.PI * 2,
...
//     // Check how much time has elapsed since transition started
//     const timeElapsed = Date.now() - (PosterTransition.startTime || Date.now());
//     const canMove = timeElapsed > 2000; // Wait 2 seconds before movement
...
//         // Animate size normalization (from poster size to game size)
//         if (ball.scaleProgress !== undefined && ball.scaleProgress < 1) {
//           ball.scaleProgress += 0.01; // Slow growth over ~100 frames (~1.6 seconds)
//           if (ball.scaleProgress > 1) ball.scaleProgress = 1;
          
//           // Ease in-out interpolation
//           const t = ball.scaleProgress;
//           const eased = t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
          
//           // Interpolate between initial and target sizes (they're already stored correctly)
//           if (ball.type === 'ball' && ball.targetRadius) {
//             // ball.radius is initial poster radius, ball.targetRadius is normalized size
//             const initialRadius = ball.targetRadius; // This was set wrong - let me think...
//             // Actually no, in init() we set ball.radius to posterSize/2 and targetRadius to normalizedSize/2
//             // So we need to keep track of the initial value
//             if (!ball.initialRadius) ball.initialRadius = ball.radius;
//             ball.radius = ball.initialRadius + (ball.targetRadius - ball.initialRadius) * eased;
//           } else if (ball.type === 'person' && ball.targetHeight) {
//             if (!ball.initialHeight) ball.initialHeight = ball.personHeight;
//             ball.personHeight = ball.initialHeight + (ball.targetHeight - ball.initialHeight) * eased;
//             ball.radius = ball.personHeight / 2; // Update clickable radius
//           }
//         }
        
//         // Start gentle movement after delay
//         if (canMove) {
...
//       // Minimum speed to keep things moving (after transition)
//       if (!PosterTransition.active) {
//         const speed = Physics.getSpeed(ball);
//         if (speed < 0.5) {
...
//       // Apply movement (only after delay)
//       if (canMove) {
//         ball.vx *= 0.99;
//         ball.vy *= 0.99;
//         ball.x += ball.vx;
...
// // ORIGINAL: GameManager with all games
// const GameManager = {
//   games: {
...
//         setTimeout(() => {
//           PosterTransition.posterWidth = posterImg.width || posterImg.naturalWidth || 1396;
//           PosterTransition.posterHeight = posterImg.height || posterImg.naturalHeight || 1606;
//           console.log('Poster dimensions updated:', PosterTransition.posterWidth, 'x', PosterTransition.posterHeight);
//         }, 100);
//       }
//     });
//   },
...
// Assets.load();
// Input.init();
// PosterTransition.init(); // This will call GameManager.init() when ready
// // GameManager.init(); // REMOVED - now called by PosterTransition
// animate();