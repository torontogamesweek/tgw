  ...
  draw() {
    ...
    this.balls.forEach((ball) => {
      ...
      if (ball.isDragging) {
        ...
        const pullDist = Math.sqrt((ball.dragStartX - ball.x) * (ball.dragStartX - ball.x) + (ball.dragStartY - ball.y) * (ball.dragStartY - ball.y));
...
class BounceGame extends BaseGame {
  ...
  spawnBall() {
    ...
    let spawnX;
  if (this.isFirstBall) {
    spawnX = this.state.target.x;  // Directly above the star
...
class MagnetZonesGame extends BaseGame {
  ...
  onPointerDown(x, y) {
    const now = Date.now(),
      ...
      distFromLastTap = Math.sqrt((x - this.state.lastTapX) * (x - this.state.lastTapX) + (y - this.state.lastTapY) * (y - this.state.lastTapY));
