...
function createParticles(position) {
  for (let i = 0; i < config.particleCount; i++) {
    ...
    particle.velocity = new THREE.Vector3((Math.random() - 0.5) * config.velocityMultiplier, (Math.random() - 0.5) * config.velocityMultiplier, 0);
...
function applyTween(target, to, duration, easing, onComplete = () => {}) {
  return new TWEEN.Tween(target).to(to, duration).easing(easing).onComplete(onComplete).start();
}
